...
model2 = pickle.load(open('churnHost1.pkl','rb'))
app = Flask(__name__)
...
@app.route('/churn')
def man3():
    return render_template('churn_home.html')
...
def churn():
    ...
    data9 = request.form['i']
    data9 = float(data9)
    arr=pd.DataFrame([[data1, data2, data3, data4 ,data5, data6, data7, data8,data9]], columns=['CreditScore','Gender','Age','Tenure','Balance','NumOfProducts','HasCrCard','IsActiveMember','EstimatedSalary'])
    pred = model2.predict(arr)
    return render_template('churnafter.html', data=pred)
